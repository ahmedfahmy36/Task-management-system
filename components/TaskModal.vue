        ...
        <div class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
          <div>
            ...
            <p class="text-sm text-slate-400 mt-0.5">
              {{ store.taskToEdit ? 'Update task details below.' : 'Fill in the details to add a new task.' }}
            </p>
          </div>
          <button
            @click="store.closeModal()"
            class="text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-full p-2 transition-colors"
          >
...
const handleSubmit = (taskData: Task | Omit<Task, 'id' | 'logs' | 'createdAt'>) => {
  if (store.taskToEdit) {
